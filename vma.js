...
function calculateFractionne(total, min, max, interval, distance, data, customLabel) {
  if(customLabel != null){
...
function init(mas){
  dataVMASpeed;
  dataFractionne = [];
  dataFractionneCompet = [];

  var MAS = mas;
  var mas_ms = MAS * 1000 / 3600;
  var total = 100 / mas_ms;
  ...
  for(distance = 100; distance < 1000; distance += 100){
    ...
    calculateFractionne(total, 65, 115, 5, distance, dataFractionne);
  }
  // 1000 a 5000
  for(distance = 1000; distance <= 5000; distance += 1000){
    calculateFractionne(total, 65, 115, 5, distance, dataFractionne);
  }

  // 10 km
  calculateFractionne(total, 65, 115, 5, 10000, dataFractionne, "10 km");
  // semi
  calculateFractionne(total, 65, 115, 5, 21100, dataFractionne, "Semi");
  // marathon
  calculateFractionne(total, 65, 115, 5, 42200, dataFractionne, "Marathon");
  // 100 km
  calculateFractionne(total, 65, 115, 5, 100000, dataFractionne, "100 km");
